...
public class BaseUpgradeSystem implements IUpgradeSystem {
    ...
    public void write(final IUpgradeItem item, final List<EnumInfoUpgradeModules> lst, ItemStack stack) {
        ...
        if (this.hasModules(EnumInfoUpgradeModules.SILK_TOUCH, stack)) {
            enchantmentMap.put(Enchantment.silkTouch, 1);
        }else{
            enchantmentMap.remove(Enchantment.silkTouch);
        }
        if (this.hasModules(EnumInfoUpgradeModules.LUCKY, stack)) {
            enchantmentMap.put(Enchantment.fortune, this.getModules(EnumInfoUpgradeModules.LUCKY, stack).number);
        }else{
            enchantmentMap.remove(Enchantment.fortune);
        }
        if (this.hasModules(EnumInfoUpgradeModules.EFFICIENT, stack)) {
            enchantmentMap.put(Enchantment.efficiency, (1 + (this.getModules(
                    EnumInfoUpgradeModules.EFFICIENT,
                    stack
            ).number - 1) * 2));
        }else{
            enchantmentMap.remove(Enchantment.efficiency);
        }
        if (this.hasModules(EnumInfoUpgradeModules.FIRE, stack)) {
            enchantmentMap.put(Enchantment.fireAspect, this.getModules(EnumInfoUpgradeModules.FIRE, stack).number);
        }else{
            enchantmentMap.remove(Enchantment.fireAspect);
        }
        if (this.hasModules(EnumInfoUpgradeModules.LOOT, stack)) {
            enchantmentMap.put(Enchantment.looting, this.getModules(EnumInfoUpgradeModules.LOOT, stack).number);
        }else{
            enchantmentMap.remove(Enchantment.looting);
        }
        if (this.hasModules(EnumInfoUpgradeModules.PROTECTION_ARROW, stack)) {
            enchantmentMap.put(Enchantment.projectileProtection, (1 + (this.getModules(
                    EnumInfoUpgradeModules.PROTECTION_ARROW,
                    stack
            ).number - 1) * 2));
        }else{
            enchantmentMap.remove(Enchantment.projectileProtection);
        }
